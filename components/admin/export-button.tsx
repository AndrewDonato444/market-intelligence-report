...
export function ExportButton({ onExportCsv, onExportJson, disabled }: ExportButtonProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div
          className="absolute right-0 mt-1 w-36 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[var(--radius-sm)] shadow-[var(--shadow-md)] z-10"
          data-testid="export-dropdown"
        >
          <button
            onClick={() => { onExportCsv(); setOpen(false); }}
            data-testid="export-csv"
            className="w-full text-left px-[var(--spacing-3)] py-[var(--spacing-2)] text-sm text-[var(--color-text)] hover:bg-[var(--color-primary-light)] transition-colors"
          >
            Export CSV
          </button>
          <button

        ...
        <>
          ...
          <Section title="API Costs by Provider">
            {data.byProvider.length === 0 ? (
              <p className="text-sm text-[var(--color-text-secondary)]">No API calls in this period.</p>
            ) : (
              <table className="w-full">
                ...
                <tbody>
                  {data.byProvider.map((p) => (
                    <tr key={p.provider} className="border-b border-[var(--color-border)] last:border-0">
                      <td className="py-[var(--spacing-2)] text-sm font-medium text-[var(--color-text)]">
                        {p.provider}
                      </td>
                      <td className="py-[var(--spacing-2)] text-sm text-right text-[var(--color-text)]">
                        {p.callCount.toLocaleString()}
                      </td>
                      <td className="py-[var(--spacing-2)] text-sm text-right font-mono text-[var(--color-text)]">
                        {formatCost(p.totalCost)}
                      </td>
                      <td className="py-[var(--spacing-2)] text-sm text-right text-[var(--color-text-secondary)]">
                        {p.cacheHits.toLocaleString()}
                      </td>
                      <td className="py-[var(--spacing-2)] text-sm text-right font-mono text-[var(--color-text-secondary)]">
              ...
              <table className="w-full">
                ...
                <tbody>
                  {data.cacheHealth.bySource.map((s) => (
                    <tr key={s.source} className="border-b border-[var(--color-border)] last:border-0">
                      <td className="py-[var(--spacing-2)] text-sm text-[var(--color-text)]">
                        {s.source}
                      </td>
                      <td className="py-[var(--spacing-2)] text-sm text-right text-[var(--color-text)]">
                        {s.entryCount}
          ...
          <Section title="Pipeline Health">
            ...
            <p className="text-sm text-[var(--color-text-secondary)] mb-[var(--spacing-3)]">
              Avg Duration: <span className="font-mono font-medium text-[var(--color-text)]">{formatDuration(data.pipelineHealth.avgDurationMs)}</span>
            </p>
            {data.pipelineHealth.recentFailures.length > 0 && (
              <div className="space-y-[var(--spacing-2)]">
                <p className="text-xs font-semibold text-[var(--color-text-secondary)]">Recent Failures</p>
          ...
          <Section
            ...
            action={
              <button
                ...
                className="px-[var(--spacing-3)] py-[var(--spacing-1)] rounded-[var(--radius-sm)] text-xs font-medium border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:bg-[var(--color-primary-light)] disabled:opacity-50 transition-colors"
              >
            ...
            <div className="space-y-[var(--spacing-2)]">
              {data.dataSources.map((ds) => {
                ...
                return (
                  <div
                    key={ds.name}
                    className="flex items-center justify-between p-[var(--spacing-3)] rounded-[var(--radius-sm)] border border-[var(--color-border)]"
                  >
                    <div className="flex items-center gap-[var(--spacing-3)]">
                      ...
                      <span className="text-sm font-medium text-[var(--color-text)]">{ds.name}</span>
                    </div>
                    <div className="flex items-center gap-[var(--spacing-4)] text-xs text-[var(--color-text-secondary)]">
...
function SummaryCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-[var(--color-surface)] rounded-[var(--radius-md)] border border-[var(--color-border)] p-[var(--spacing-4)]">
      ...
      <p className="text-xl font-semibold text-[var(--color-text)] mt-1 font-[family-name:var(--font-sans)]">{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="bg-[var(--color-surface)] rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] border border-[var(--color-border)] p-[var(--spacing-4)]">
      <div className="flex items-center justify-between mb-[var(--spacing-4)]">
        <h2 className="font-[family-name:var(--font-sans)] text-sm font-semibold text-[var(--color-text)]">
          {title}
        </h2>
        {action}
      </div>
...
function MiniStat({ label, value, warn }: { label: string; value: string; warn?: boolean }) {
  return (
    <div className="text-center">
      ...
      <p className={`text-lg font-semibold mt-0.5 ${warn ? "text-[var(--color-error)]" : "text-[var(--color-text)]"}`}>
        {value}
      </p>
    </div>
  );
...
function Th({ children, align }: { children: React.ReactNode; align: "left" | "right" }) {
  return (
    <th className={`py-[var(--spacing-2)] text-xs font-semibold text-[var(--color-text-secondary)] text-${align}`}>
      {children}
    </th>
  );
}

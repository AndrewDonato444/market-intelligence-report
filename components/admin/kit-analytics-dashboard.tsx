        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-[var(--spacing-4)]">
            ...
            <div className="bg-[var(--color-app-surface)] rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] border border-[var(--color-app-border)] p-[var(--spacing-4)]">
              <h2 className="font-[family-name:var(--font-body)] text-sm font-semibold text-[var(--color-app-text)] mb-[var(--spacing-3)]">
              ...
              <div className="space-y-[var(--spacing-2)]">
                {data.topContentTypes.map((type) => (
                  <div key={type} className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-semibold text-[var(--color-app-text)] font-mono">
          ...
          <div className="bg-[var(--color-app-surface)] rounded-[var(--radius-md)] shadow-[var(--shadow-sm)] border border-[var(--color-app-border)] p-[var(--spacing-4)]">
            ...
            {data.volumeOverTime.length > 0 ? (
              <div className="space-y-[var(--spacing-1)]">
                {data.volumeOverTime.map((entry) => (
                  <div key={entry.date} className="flex items-center gap-[var(--spacing-3)]">
                    <span className="text-xs text-[var(--color-app-text-secondary)] w-24 shrink-0 font-mono">
                      {new Date(entry.date + "T00:00:00Z").toLocaleDateString("en-US", { month: "short", day: "numeric", timeZone: "UTC" })}
                    </span>
                    <div className="flex-1 h-5 bg-[var(--color-app-active-bg,rgba(15,23,42,0.05))] rounded-[var(--radius-sm)] overflow-hidden">
                      <div
                        className="h-full bg-[var(--color-app-accent)] rounded-[var(--radius-sm)]"
                        style={{
                          width: `${Math.max(4, (entry.count / Math.max(...data.volumeOverTime.map((e) => e.count), 1)) * 100)}%`,
                        }}
                      />
                    </div>
                    <span className="text-xs font-semibold text-[var(--color-app-text)] w-8 text-right font-mono">
                      {entry.count}
                    </span>
                  </div>
                ))}
              ...
              <p className="text-sm text-[var(--color-app-text-secondary)] text-center py-[var(--spacing-8)]">

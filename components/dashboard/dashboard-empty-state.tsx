...
export function DashboardEmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-[var(--spacing-16)] px-[var(--spacing-6)]">
      <div className="w-40 h-32 rounded-[var(--radius-md)] border-2 border-[var(--color-accent)] border-dashed flex items-center justify-center mb-[var(--spacing-8)]">
        <svg
          width="64"
          height="64"
          viewBox="0 0 64 64"
      ...
      <p className="font-[family-name:var(--font-sans)] text-base text-[var(--color-text-secondary)] text-center max-w-md mt-[var(--spacing-3)]">
        Your intelligence platform is ready. Start by telling us about the market you advise in.
      </p>

      <Link
        href="/reports/create"
        className="mt-[var(--spacing-6)] inline-flex items-center justify-center px-6 py-3 bg-[var(--color-accent)] text-[var(--color-primary)] font-[family-name:var(--font-sans)] font-semibold rounded-[var(--radius-sm)] hover:bg-[var(--color-accent-hover)] transition-colors"
      >
        Get Started
      </Link>
      ...
      <div
        ...
        className="w-48 h-0.5 bg-[var(--color-accent)] mt-[var(--spacing-6)]"
      />
    </div>
  );
}
